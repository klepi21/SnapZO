...
export async function POST(req: Request) {
  ...
  if (typeof assets !== "string" || typeof nonce !== "string" || typeof deadline !== "string") {
    return NextResponse.json({ error: "Missing assets, nonce, or deadline" }, { status: 400 });
  }
  ...
  try {
    const est = await publicClient.estimateContractGas({
      ...
      args: [userAddr, assetsBi, nonceBi, deadlineBi, signature],
      account: account.address,
    });
    gas = est + (est * BigInt(30)) / BigInt(100);
    if (gas < BigInt(450_000)) {
      gas = BigInt(450_000);
    }
  } catch {
    gas = BigInt(600_000);
